...
///
/// **SORTIE BRUTE ET REDESSIN PARTENT PAR LE MEME EVENEMENT**, et le frontend les donne tels
/// quels a xterm, qui n'a pas a savoir lequel des deux il recoit. Un drapeau `redessin` a
/// existe le temps des reperes d'agent (0.83.0 a 0.86.2) : il servait a ne pas compter un
/// redessin comme une ecriture de l'agent. Ces reperes ont ete retires, et lui avec.
...
fn emettre_la_sortie(contexte: &Contexte, id: i64, octets: &[u8]) {
    contexte.emetteur.emettre(
        "terminal_output",
        serde_json::to_value(SortiePayload { id, data: b64(octets) }).unwrap_or_default(),
    );
}
...
fn traiter_poussee(contexte: &Contexte, pousse: Pousse) {
    match pousse {
        ...
        Pousse::Sortie { id, octets } | Pousse::Redessin { id, octets } => {
            emettre_la_sortie(contexte, id, &octets)
        }
